...
// créer un liste d'arête à partir la matrice d'adjacent du graphe G 
//et retourne le nombre totale d'arête
int creatEdgesG(Graph& G) {
    int nbE=0;  // nombre d'arête
    Edge* p;
    for (int i = 0; i < N; i++)
      for (int j = i + 1; j < N; j++){
	      if (G.g[i][j] != INF) {
...
// Ajouter un arête e dans un arbre a
...
// supprimer un arête e du arbre a
...
// trouver des arêtes qui sont de poids maximum parmis les arêtes pour allant de point x au point s
...
// trouver de l'arbre couvrant en tester s'il est satisfait la limitation
